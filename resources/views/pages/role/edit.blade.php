...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui data role</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <form action="{{ route('role.update', $role->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name', $role->name) }}" placeholder="contoh: Admin, Kasir"
                            ...
                            <div class="space-y-2">
                                @foreach($group['actions'] as $actionKey => $label)
                                    ...
                                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                        <input type="checkbox" name="permissions[]" value="{{ $permValue }}" class="permission-checkbox h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600" {{ in_array($permValue, $currentPermissions ?? []) ? 'checked' : '' }} />
                                        <span>{{ $label }}</span>
                                    </label>
                                @endforeach
                            </div>
        ...
        <div class="mt-4">
            <form action="{{ route('role.destroy', $role->id) }}" method="POST" data-confirm-message="Apakah Anda yakin ingin menghapus role ini?">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Hapus Role</button>
            </form>
        </div>
    </div>
</div>
